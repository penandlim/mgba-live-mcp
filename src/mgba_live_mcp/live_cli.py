...
def prepare_bridge_script(session_scripts_dir: Path) -> Path:
    ...
    session_bridge = session_scripts_dir / BRIDGE_SCRIPT.name
    try:
        shutil.copy2(BRIDGE_SCRIPT, session_bridge)
    except OSError as exc:
        raise SystemExit(f"Failed to stage bridge script in session dir: {session_bridge}") from exc
...
def build_start_command(
    *,
    ...
    bridge_path = BRIDGE_SCRIPT if bridge_script is None else bridge_script
    cmd = [
        mgba_path,
        "-C",
...
def cmd_start(args: argparse.Namespace) -> None:
    ...
    session_bridge_script = prepare_bridge_script(scripts_dir)
